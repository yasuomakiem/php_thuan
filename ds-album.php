    ...
    <div id="right">
        ...
        <div class="danhsach">
            <table  cellspacing="0" cellpadding="0">
                <tr>
                    <th>TT</th><th>Tên album</th><th>Ảnh đại diện</th><th>Trong mục</th><th>Quyền hạn</th>
                </tr>
                <?php 
                $tsp="select * from dh_baiviet where loai=3 order by time desc";$qsp=mysql_query($tsp);$soluong=mysql_num_rows($qsp);
                //phan trang
                ...
                <tr>
                    <td><?php echo $i; ?></td>
                    <td style="background: #F3F3F3;"><?php echo $rsp['ten']; ?><br /><br />
                    <?php 
                        if($r['t4']!='' and $r['t4e']=='3'){if($rsp['t2']==1){echo '<a style="padding: 5px 8px;background:#0060BF;border-radius: 4px;color:white" title="Nhấn để tắt" href="up-sp.php?loai=t2&up=0&id='.$rsp['id'].'">'.$r['t4'].'</a>&nbsp;';}else{echo '<a style="padding: 5px 8px;border-radius: 4px;background:gray;color:white" title="Nhấn để mở" href="up-sp.php?loai=t2&up=1&id='.$rsp['id'].'">'.$r['t4'].'</a>&nbsp;';}}
                        if($r['t5']!='' and $r['t5e']=='3'){if($rsp['t3']==1){echo '<a style="padding: 5px 8px;background:#0060BF;border-radius: 4px;color:white" title="Nhấn để tắt" href="up-sp.php?loai=t3&up=0&id='.$rsp['id'].'">'.$r['t5'].'</a>&nbsp;';}else{echo '<a style="padding: 5px 8px;border-radius: 4px;background:gray;color:white" title="Nhấn để mở" href="up-sp.php?loai=t3&up=1&id='.$rsp['id'].'">'.$r['t5'].'</a>&nbsp;';}}
                    ?>
                    </td>
                    <td>
                    ...
                    <?php 
                        $m1=$rsp['muc'];
                        $tg1=mysql_query("select * from dh_menu1 where id=$m1");$tr1=mysql_fetch_assoc($tg1);echo $tr1['ten'];
                        $m2=$rsp['muc2'];
                        if($m2!=0){
                        $tg1=mysql_query("select * from dh_menu2 where id=$m2");$tr1=mysql_fetch_assoc($tg1);echo '<br />>'.$tr1['ten'];
                        }
                    ?>
                    </td>
                    <td style="line-height: 24px;">
                    <a class="sua" href="edit-album.php?edit=<?php echo $rsp['id']; ?>">Sửa</a> <br /> <a class="xoa" href="<?php echo $xoa; ?>">Xóa</a><br />
                    <a style="color: #0080FF;" href="add-img.php?id=<?php echo $rsp['id']; ?>">Quản lý ảnh</a> 
                    </td>
                </tr>
                <?php 
                } 
