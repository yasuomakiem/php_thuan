<?
echo '<div class="tit">Cài đặt Tab Ảnh</div>';
if(isset($_POST['thongtintab'])){
                    ...
                    $t1=addslashes($_POST['t1']);
                        ...
                        $in="update set_home set ten=N'$ten',bg='$tenanhbg',color='$color',t1='$t1' where id=$re[id]";
        ...
        <form id="form" action="" method="post"  enctype="multipart/form-data">
            ...
            <?php echo $thongbao; ?>
                ...
                <tr>
                    ...
                    <td>
                    <select name="t1">
                        <option value="" <?if($re['t1']==''){echo 'selected=""';}?>>Full màn hình</option>
                        <option value="nofull" <?if($re['t1']=='nofull'){echo 'selected=""';}?>>Trong khuôn khổ trang</option>
                        
                    </select>
                    </td>
                </tr>
                <tr>
                    <th></th><td><input type="submit" name="thongtintab" value="CẬP NHẬT" /></td>
                </tr>
                <tr>
                    <th></th><td><b>Lưu ý: </b> Chế độ "Hiển thị" chỉ có tác dụng khi bạn thêm 1 ảnh duy nhất<br />
                    Lúc này full màn hình là để nó tràn ra tận mép ngoài cùng của màn hình, còn không nó chỉ rộng bằng giới hạn trang.<br />
                    Khi 1 ảnh và full trang thì màu nền và hình nền không còn tác dụng nữa.
                    </td>
                </tr>
            </table>
        </form>
               <? 
            echo '<div class="tit">Thêm các Ảnh</div>';
            if(isset($_GET['id'])){
                ...
                if(isset($_POST['taobanner'])){
                    ...
                    $a=addslashes($_POST['a']);
                        ...
                        move_uploaded_file($_FILES['image']['tmp_name'],"upload/banner/".$tenanh);
                        // xong ảnh
                        $in="insert into dh_banner (ten,idvitri,anh,a,target,time,iduser)value
            ...
            <?php echo $thongbao; ?>
                <tr>
                    <th><span>*</span>Tên banner:</th><td><input required="" name="ten" value="<?php echo $ten; ?>" /></td>
                </tr>
        ...
        <div id="right">
        <div class="tit">Danh sách ảnh </div>
        <div class="danhsach">
            <table  cellspacing="0" cellpadding="0">
                ...
                <?php 
                $tsp="select * from dh_banner where idvitri=$edit and iduser=$iduser order by time desc";$qsp=mysqli_query($con,$tsp);$soluong=mysqli_num_rows($qsp);
